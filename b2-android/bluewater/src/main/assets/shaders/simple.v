...
float getPointSize(vec3 position, float radius);

void main()
{
	vec4 position = in_projection * in_modelview * vec4(in_pos, 1.0);

	point_size = in_point_size * getPointSize(in_pos, 0.5);
...
float getPointSize(vec3 position, float radius)
{
	...
	vec2 p1 = vec2(projectPoint(in_projection * in_modelview * vec4(in_pos, 1.0))),
		p2 = vec2(projectPoint(in_projection * in_modelview * vec4(in_pos + right * radius, 1.0)));
